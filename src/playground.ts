...
function scrollTween(offset) {
    return function () {
        let i = d3.interpolateNumber(window.pageYOffset ||    // windows.pageYOffset 返回文档在垂直方向已滚动的像素值
...
let boundary: { [id: string]: { [type: string]: number[][] } } = {};
...
function makeGUI() {
    ...
    d3.select("#reset-button").on("click", () => {
        reset();
        userHasInteracted();  // 只在用户第一次 interact 时执行
    ...
    player.onPlayPause(isPlaying => {
        // classed is used to set the class (playing) to the selected element (#play-pause-button)
    ...
    d3.select("#next-step-button").on("click", () => {
        ...
        userHasInteracted();
        if (iter === 0) {
            simulationStarted();  // Google analysis
        }
        oneStep();
    });
    ...
    d3.select("#data-regen-button").on("click", () => {
        generateData();
        parametersChanged = true;   // Google analysis
    });

    // 绑定数据缩略图元素的点击事件：更换数据集
    let dataThumbnails = d3.selectAll("canvas[data-dataset]");  // select canvas that have a data-dataset attribute
    dataThumbnails.on("click", function () {
        ...
        dataThumbnails.classed("selected", false);  // 先 unset 之前 selected 数据集
        d3.select(this).classed("selected", true);  // 再 set 被选中数据集
        generateData();
    ...
    discretize.property("checked", state.discretize); // ensures that the checkbox is checked or unchecked

    // test data ratio（10%~100%）
    ...
    // Learning rate function selector
...
function drawNode(cx: number, cy: number, nodeId: string, isInput: boolean,
    ...
    if (!isInput) {
        ...
        // Draw the node's normalization result
...
// Draw the node's canvas.
    let div = d3.select("#network").insert("div", ":first-child")
        ...
        .on("mouseenter", function () {
            ...
            updateDecisionBoundary(network, false);
            heatMap.updateBackground(boundary[nodeId]['non_norm'], state.discretize);
        })
        .on("mouseleave", function () {
            selectedNodeId = null;
...
// draw node heatmap
    let nodeHeatMap = new HeatMap(RECT_SIZE, DENSITY / 10, xDomain,
        xDomain, div, {noSvg: true});
    div.datum({heatmap: nodeHeatMap, id: nodeId, type: 0});  // bond the attr to use later
}

// Draw network
function drawNetwork(network: nn.Node[][]): void {
    let svg = d3.select("#svg");  // 整个html网页唯一 id 为 svg 的元素
    ...
    // Get the width of the svg container. important!
    let padding = 3;
    ...
    let container = svg.append("g")
        .classed("core", true)
        .attr("transform", `translate(${padding},${padding})`); // 向右向下分别移动 padding

    // Draw the network layer by layer.
    let numLayers = network.length;
    let featureWidth = 100;
    let layerScale = d3.scale.ordinal<number, number>()     // 每一层起点的 scale
...
function easyHover(type: Number, content?: String, coordinates?: [number, number]) {
    let hoverCard = d3.select("#easyHover")
    if (type == null) {
    ...
    // @ts-ignore
    hoverCard.select(".value").text(content);
}
...
function updateDecisionBoundary(network: nn.Node[][], firstTime: boolean) {
    if (firstTime) {
        ...
        nn.forEachNode(network, true, node => {
            boundary[node.id] = {}
            boundary[node.id]['non_norm'] = new Array(DENSITY);
            boundary[node.id]['norm'] = new Array(DENSITY);
        });
        // Go through all predefined inputs.
        for (let nodeId in INPUTS) {
            boundary[nodeId] = {}
            boundary[nodeId]['non_norm'] = new Array(DENSITY);
        }
    }
    ...
    for (i = 0; i < DENSITY; i++) {
        if (firstTime) {
            nn.forEachNode(network, true, node => {
                boundary[node.id]['non_norm'][i] = new Array(DENSITY);
                boundary[node.id]['norm'][i] = new Array(DENSITY);
            });
            // Go through all predefined inputs.
            for (let nodeId in INPUTS) {
        ...
        for (j = 0; j < DENSITY; j++) {
            ...
            if (firstTime) {
                // Go through all predefined inputs.
                for (let nodeId in INPUTS) {
                    boundary[nodeId]['non_norm'][i][j] = INPUTS[nodeId].f(x, y);
                }
            }
        }
    }
...
function updateUI(firstStep = false) {
    ...
    heatMap.updateBackground(boundary[selectedId]['non_norm'], state.discretize);

    // Update all decision boundaries.
    d3.select("#network").selectAll("div.canvas")
        .each(function (data: { heatmap: HeatMap, id: string, type: number }) {
            if (data.type == 0) {
                data.heatmap.updateBackground(reduceMatrix(boundary[data.id]['non_norm'], 10),
                    state.discretize);
            } else if (state.normalization != normalizations['none']) {
                data.heatmap.updateBackground(reduceMatrix(boundary[data.id]['norm'], 10),
                    state.discretize);
            }
        });
...
function drawDatasetThumbnails() {
    function renderThumbnail(canvas, dataGenerator) {
        ...
        let context = canvas.getContext("2d");    // "2d", 建立一个 CanvasRenderingContext2D 二维渲染上下文。
        let data = dataGenerator(200, 0);
        data.forEach(function (d) {
            context.fillStyle = colorScale(d.label);    // 矩形填充颜色
            context.fillRect(w * (d.x + 6) / 12, h * (d.y + 6) / 12, 4, 4); // 矩形开始点即高度和宽度
        });
        d3.select(canvas.parentNode).style("display", null);  // 源码是将所有的随机数据都初始化，但是display都设置为none
    }
    ...
    if (state.problem === Problem.REGRESSION) { // 有点怪的是，初始化的时候，这部分的数据缩略图的html元素也出现了
...
function generateData(firstTime = false) {
    ...
    heatMap.updatePoints(trainData);  // update g.train
    heatMap.updateTestPoints(state.showTestData ? testData : []); // update g.test
}

let firstInteraction = true;
let parametersChanged = false;

function userHasInteracted() {    //  ensure that the function is only executed once, on the first user interaction
    if (!firstInteraction) {
        return;
    }
...
drawDatasetThumbnails();  // 绘制缩略图
initTutorial();           // 初始化 tutorial 部分
makeGUI();                // 构建可视化界面
generateData(true);   // 创建数据
reset(true);    //
hideControls();           // 隐藏所选控制选项
